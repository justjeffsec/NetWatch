...
const COLORS = [
  "hsl(38,100%,52%)", "hsl(185,85%,42%)", "hsl(270,65%,60%)", "hsl(20,100%,55%)",
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="border font-mono text-[11px] px-3 py-2 shadow-lg"
      style={{ background:"hsl(26 90% 4%)", borderColor:"rgba(255,154,0,0.25)", borderRadius:"2px", boxShadow:"0 0 10px rgba(255,154,0,0.15)" }}>
      <p className="text-[10px] tracking-wider uppercase mb-1" style={{ color:"hsl(38 40% 45%)" }}>{label}</p>
...
const SectionHeader = ({ title, sub }: { title: string; sub?: string }) => (
  <div className="flex items-center gap-2">
    <div className="w-1 h-4 rounded-sm" style={{ background:"hsl(38 100% 52%)", boxShadow:"0 0 6px rgba(255,154,0,0.5)" }} />
    ...
    {sub && (
      <span className="border text-[9px] px-1.5 py-0.5 font-mono tracking-widest"
        style={{ borderRadius:"2px", borderColor:"rgba(255,154,0,0.2)", background:"rgba(255,154,0,0.06)", color:"hsl(38 50% 50%)" }}>
        {sub}
      </span>
    )}
  </div>
...
export default function TrafficAnalysis() {
  ...
  const axisStyle = { fontSize: 9, fill: "hsl(38 35% 38%)", fontFamily: "'Share Tech Mono',monospace" };
  const axisLineStyle = { stroke: "rgba(255,154,0,0.15)" };
      ...
      <div className="sticky top-0 z-50 border-b border-border/60"
        ...
        <div className="flex items-center justify-between px-4 py-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="h-4 w-px bg-border/60" />
            <span className="text-[11px] font-mono tabular-nums" style={{ color:"hsl(38 100% 60%)", textShadow:"0 0 8px rgba(255,154,0,0.5)" }}>
              {clock}
            </span>
          </div>
        </div>
                ...
                {timelineData.length === 0 ? <EmptyState msg="Collecting flow data..." /> : (
                  <ResponsiveContainer width="100%" height={240}>
                    <BarChart data={timelineData} barCategoryGap="15%">
                      ...
                      <Tooltip content={<ChartTooltip />} cursor={{ fill:"rgba(255,154,0,0.05)" }} />
                      <Bar dataKey="download" name="Download" fill="hsl(185,85%,42%)" radius={[2,2,0,0]} />
                      <Bar dataKey="upload"   name="Upload"   fill="hsl(38,100%,52%)"  radius={[2,2,0,0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <PieChart>
                      <Pie data={byService.slice(0, 8)} dataKey="totalBytes" nameKey="service"
                        ...
                        strokeWidth={1} stroke="hsl(26 90% 4%)">
                      ...
                      <Tooltip content={({ active, payload }: any) => {
                        ...
                        return (
                          <div className="border font-mono text-[11px] px-2 py-1.5"
                            ...
                            <span style={{ color:"hsl(38 80% 65%)" }}>{d.service}</span>: {formatBytes(d.totalBytes)}
                          </div>
                        );
                      }} />
                    </PieChart>
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 px-2 mt-2">
                    {byService.slice(0, 8).map((s: any, i: number) => (
                      <div key={s.service} className="flex items-center gap-1.5">
                        ...
                        <span className="ml-auto text-[10px] font-mono tabular-nums" style={{ color:"hsl(38 70% 55%)" }}>{formatBytes(s.totalBytes)}</span>
                      </div>
                    ))}
                  </div>
                </>
              ...
              {topTalkers.length === 0 ? <EmptyState msg="No flow data yet" /> : (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart
                    ...
                    <Tooltip content={<ChartTooltip />} cursor={{ fill:"rgba(255,154,0,0.05)" }} />
                    <Bar dataKey="download" name="Download" fill="hsl(185,85%,42%)" radius={[0,2,2,0]} stackId="a" />
                    <Bar dataKey="upload"   name="Upload"   fill="hsl(38,100%,52%)"  radius={[0,2,2,0]} stackId="a" />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </CardContent>
              ...
              <ScrollArea className="h-[265px]">
                {byDevice.length === 0 ? <EmptyState msg="No device data yet" /> : (
                  <table className="w-full">
                    <thead>
                      <tr className="border-b" style={{ borderBottomColor:"rgba(255,154,0,0.12)" }}>
                      ...
                      {byDevice.slice(0, 30).map((d: any) => (
                        <tr key={d.ip} className="border-b transition-colors group"
                          ...
                          <td className="px-4 py-2">
                            <div className="font-mono text-[11px]" style={{ color:"hsl(38 80% 65%)" }}>
                              {d.label || d.ip}
                            </div>
                            {d.label && <div className="text-[9px] font-mono opacity-40">{d.ip}</div>}
                            {d.org   && <div className="text-[9px] font-mono opacity-35">{d.org}</div>}
                          </td>
                          <td className="text-right px-3 py-2 font-mono text-[11px] tabular-nums" style={{ color:"hsl(185 80% 50%)" }}>
                            {formatBytes(d.bytesIn)}
                          </td>
                          <td className="text-right px-3 py-2 font-mono text-[11px] tabular-nums" style={{ color:"hsl(38 100% 55%)" }}>
                            {formatBytes(d.bytesOut)}
                          </td>
                          <td className="text-right px-3 py-2 font-mono text-[11px] tabular-nums font-medium" style={{ color:"hsl(38 80% 65%)" }}>
