...
const threatStyle = (level: string | null) => {
  switch (level) {
    ...
    case "suspicious": return { borderColor: "rgba(0,230,65,0.4)",  background: "rgba(0,230,65,0.1)", color: "hsl(128 100% 55%)" };
    ...
    <Card className="border-card-border">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            <div className="w-1 h-4 rounded-sm"
              style={{ background: maliciousCount > 0 ? "#ff4040" : "hsl(128 100% 45%)", boxShadow: maliciousCount > 0 ? "0 0 6px rgba(255,40,40,0.7)" : "0 0 6px rgba(0,230,65,0.5)" }} />
      ...
      <CardContent className="px-4 pb-2 pt-1">
        <div className="relative mb-2">
          <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3" style={{ color: "hsl(128 40% 36%)" }} />
          <Input
            value={filter}
      ...
      <CardContent className="px-0 pb-0 pt-0">
        <ScrollArea className="h-[270px]">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10" style={{ color: "hsl(128 30% 34%)" }}>
            ...
            <div>
              {filtered.map((device) => (
                <div key={device.id}
                  ...
                  style={{
                    borderBottomColor: "rgba(0,230,65,0.07)",
                  ...
                  <div className="flex-shrink-0">
                    {device.threatLevel === "malicious"
                      ...
                      : device.threatLevel === "suspicious"
                        ? <ShieldAlert className="w-4 h-4" style={{ color: "hsl(128 100% 52%)", filter: "drop-shadow(0 0 3px rgba(0,230,65,0.7))" }} />
                        : device.trusted
                          ? <ShieldCheck className="w-4 h-4" style={{ color: "#4ade80" }} />
                          : <ShieldAlert className="w-4 h-4" style={{ color: "hsl(128 50% 45%)" }} />
                    }
                  </div>
                    ...
                    {editingId === device.id ? (
                      <div className="flex items-center gap-1">
                        ...
                        <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => setEditingId(null)}>
                          <X className="w-3 h-3" style={{ color: "hsl(128 40% 40%)" }} />
                        </Button>
                      </div>
                    ) : (
                      <>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-[11px] font-mono" style={{ color: "hsl(128 80% 60%)" }}>
                            {device.ipAddress}
                          </span>
                          {device.label && (
                            <span className="border text-[9px] px-1 py-0 font-mono"
                              style={{ borderRadius:"2px", borderColor:"rgba(0,200,100,0.3)", background:"rgba(0,200,100,0.07)", color:"hsl(150 80% 52%)" }}>
                              {device.label}
                            </span>
                          )}
                          {device.country && (
                          ...
                          <button className="opacity-0 group-hover:opacity-100 transition-opacity"
                            ...
                            <Pencil className="w-2.5 h-2.5" style={{ color: "hsl(128 40% 40%)" }} />
                          </button>
                        </div>
                  ...
                  <Button variant="ghost" size="sm"
                    ...
                    style={{ color: device.trusted ? "#4ade80" : "hsl(128 35% 40%)" }}
