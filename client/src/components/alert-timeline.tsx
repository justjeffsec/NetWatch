      ...
      <CardHeader className="pb-2 pt-3 px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Clock className="w-3 h-3 opacity-30" style={{ color: "hsl(38 50% 50%)" }} />
          </div>
        </div>
      </CardHeader>
        ...
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height={150}>
            <BarChart data={data} barCategoryGap="15%">
              <XAxis dataKey="time"
                tick={{ fontSize: 9, fill: "hsl(38 35% 38%)", fontFamily: "'Share Tech Mono',monospace" }}
                axisLine={{ stroke: "rgba(255,154,0,0.15)" }}
                tickLine={false}
                interval="preserveStartEnd" />
              <YAxis
                tick={{ fontSize: 9, fill: "hsl(38 35% 38%)", fontFamily: "'Share Tech Mono',monospace" }}
                axisLine={false} tickLine={false} allowDecimals={false} width={22} />
              <Tooltip content={<CustomTooltip />} cursor={{ fill: "rgba(255,154,0,0.05)" }} />
              <Bar dataKey="critical" stackId="a" fill="#ff3030" radius={[0,0,0,0]} />
              <Bar dataKey="warning"  stackId="a" fill="hsl(38,100%,52%)" radius={[0,0,0,0]} />
              <Bar dataKey="info"     stackId="a" fill="hsl(185,80%,42%)" radius={[2,2,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </CardContent>
